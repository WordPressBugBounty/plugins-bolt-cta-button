...
class CNCB_Frontend {
	...
	private function add_inline_styles( $tpl ) {
		...
		if ( 'bar' === $tpl['mobile'] || 'bar' === $tpl['desktop'] ) {
			...
			$css .= sprintf(
				'#cncb-bar{background-color:rgba(%d,%d,%d,%.2f) !important;padding:%dpx !important;%s:0;margin:%dpx %dpx %dpx %dpx !important;}',
				$r, $g, $b, $alpha, $pad, $pos, $mt, $mr, $mb, $ml
			);
			$css .= sprintf( '#cncb-bar .cncb-btn{border-radius:%dpx !important;}', $rad );

			// Per-button colors (high specificity + !important to win over themes).
			$i = 0;
			foreach ( $this->get_enabled_buttons() as $btn ) {
				...
				$css .= sprintf(
					'#cncb-bar .cncb-btn-%d{background-color:%s !important;color:%s !important;}#cncb-bar .cncb-btn-%d:hover{background-color:%s !important;color:%s !important;}',
					$i, $bgc, $txt, $i, $bgc, $txt
				);
				$i++;
			}
		...
		if ( 'fab' === $tpl['mobile'] || 'fab' === $tpl['desktop'] ) {
			...
			$css .= sprintf(
				...
				$fab_bg, $fab_txt, $fab_bg, $fab_txt, $fab_txt, $fab_txt
			);
		}
	...
	private function render_bar( $buttons, $extra_class = '' ) {
		...
		$bar_shadow = isset( $o['bar_shadow'] ) && in_array( $o['bar_shadow'], array( 'none', 'sm', 'md', 'lg' ), true ) ? $o['bar_shadow'] : 'none';
		$classes[] = 'cncb-shadow-' . $bar_shadow;

		$bar_btn_shadow = isset( $o['bar_button_shadow'] ) && in_array( $o['bar_button_shadow'], array( 'none', 'sm', 'md', 'lg' ), true ) ? $o['bar_button_shadow'] : 'none';
		$classes[] = 'cncb-btn-shadow-' . $bar_btn_shadow;
	...
	private function render_fab( $buttons, $extra_class = '' ) {
		...
		$main_anim = isset( $o['fab_animation'] ) && in_array( $o['fab_animation'], array( 'pulse', 'glow', 'bounce', 'shake' ), true ) ? $o['fab_animation'] : 'none';
		$shadow = isset( $o['fab_shadow'] ) && in_array( $o['fab_shadow'], array( 'none', 'sm', 'md', 'lg' ), true ) ? $o['fab_shadow'] : 'none';
		...
		$classes = array(
			...
			'cncb-fab-anim-' . $anim,
			'cncb-fab-size-' . $size,
			'cncb-fab-shadow-' . $shadow,
		);
		if ( 'none' !== $main_anim ) {
			$classes[] = 'cncb-fab-main-anim-' . $main_anim;
		}
				...
				<?php foreach ( $buttons as $btn ) :
					...
					<div class="cncb-fab-sub" role="none">
						<a href="<?php echo esc_url( $btn['url'] ); ?>"
						   ...
						   <?php echo $rel ? 'rel="' . esc_attr( $rel ) . '"' : ''; ?>
						   data-button-index="<?php echo esc_attr( $btn['_index'] ); ?>"
						   style="background-color:<?php echo esc_attr( $bg ); ?> !important;color:<?php echo esc_attr( $txt ); ?> !important;"
						   aria-label="<?php echo esc_attr( $btn['label'] ); ?>">
							<?php $this->render_icon( $btn['icon'] ); ?>
						</a>
						<?php if ( $show_tt ) : ?>
