...
(function ($) {
	...
	var CNCB_Admin = {
		...
		bindEvents: function () {
			...
			// ---- Auto-bind ALL form inputs in the settings tabs to live preview ----
			// This catches every <input>, <select>, <textarea> change/input event,
			// so any new field added to the admin UI is reflected in the preview
			// without needing to update this list.
			$(document).on('change input', '#cncb-tabs input, #cncb-tabs select, #cncb-tabs textarea', function () {
				self.updatePreview();
			});
			...
			// ---- Reset to Defaults (delegated for robustness) ----
			$(document).on('click', '#cncb-reset-options', function (e) {
				e.preventDefault();
				var btn = $(this);
				if (!window.confirm(btn.data('confirm'))) {
					return false;
				}
				var status = $('#cncb-reset-options-status');
				btn.prop('disabled', true);
				status.text(btn.data('resetting'));
				$.post(ajaxurl, {
					action: 'cncb_reset_options',
		...
		collectOptions: function () {
			...
			var options = {
				...
				bar_shadow: $('#cncb-bar-shadow').val() || 'none',
				bar_button_shadow: $('#cncb-bar-button-shadow').val() || 'none',
				...
				fab_animation: $('#cncb-fab-animation').val() || 'none',
				fab_shadow: $('#cncb-fab-shadow').val() || 'none',
		...
		renderBarPreview: function (options) {
			...
			// Two independent shadows: bar background + per-button
			var barShadowMap = {
				'none': 'none',
				'sm':   '0 -1px 3px rgba(0,0,0,0.10)',
				'md':   '0 -2px 12px rgba(0,0,0,0.16)',
				'lg':   '0 -6px 24px rgba(0,0,0,0.24)'
			};
			var barShadowMapTop = {
				'none': 'none',
				'sm':   '0 1px 3px rgba(0,0,0,0.10)',
			...
			var btnShadowMap = {
				'none': 'none',
				'sm':   '0 1px 3px rgba(0,0,0,0.10), 0 1px 2px rgba(0,0,0,0.08)',
				'md':   '0 2px 8px rgba(0,0,0,0.15)',
				'lg':   '0 4px 14px rgba(0,0,0,0.25)'
			};
			var barShadowKey = options.bar_shadow || 'none';
			var barShadow = (options.bar_position === 'top') ? barShadowMapTop[barShadowKey] : barShadowMap[barShadowKey];
			var btnShadowKey = options.bar_button_shadow || 'none';
			var btnShadow = btnShadowMap[btnShadowKey] || 'none';
		...
		renderFabPreview: function (options) {
			...
			var shadowMap = {
				...
				'md':   '0 4px 14px rgba(0,0,0,0.18)',
				'lg':   '0 10px 28px rgba(0,0,0,0.28)'
			};
			var fabShadowKey = options.fab_shadow || 'none';
			var fabAnimKey = options.fab_animation || 'none';

			var $mainBtn = $('<div class="cncb-fab-main-btn"></div>');
			if (fabAnimKey !== 'none') {
				$mainBtn.addClass('cncb-fab-main-anim-' + fabAnimKey);
			}
			$mainBtn.css({
				'width': fabSize + 'px',
				'height': fabSize + 'px',
				...
				'justify-content': 'center',
				'cursor': 'pointer',
				'box-shadow': shadowMap[fabShadowKey] || 'none'
			});
